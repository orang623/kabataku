...
function endGame() {
    ...
    startBtn.textContent = "Mulai Mode Kompetitif";
    
    setTimeout(() => {
        const playerName = prompt(`Waktu Habis! Skor Benar Anda: ${correctCount}. \nMasukkan nama Anda untuk Leaderboard:`);
        
        if (playerName && playerName.trim() !== "") {
            saveToLeaderboard(playerName, correctCount);
        } else {
            alert("Skor tidak disimpan karena nama tidak diisi.");
        }
    }, 500);
}
...
function answer(e) {
    ...
    if (e.currentTarget.textContent === baku[lastIndex]) {
        answerText.textContent = `Jawaban Anda, "${e.currentTarget.textContent}", benar!`;
        answerText.classList.remove('wrong');
        answerText.classList.add('correct');
        correctCount += 1;
        correctStat.textContent = correctCount;
    } else {
        answerText.textContent = `Jawaban Anda, "${e.currentTarget.textContent}", salah!`;
